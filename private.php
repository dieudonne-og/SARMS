...
<?php $sql = "SELECT message.ongoing,message.SubjectName,message.type,message.Creationdate,message.classid, admin.classid,admin.UserName from message join admin on admin.UserName=message.ongoing where message.classid='' and SubjectCode=1 ORDER BY message.id DESC  ";
$query = $dbh->prepare($sql);
$query->execute();
...
<?php $cnt=$cnt+1; ?>
     ...
     <?php
    $sql = "SELECT classid,UserName from  admin where UserName=''"    ;
$query = $dbh->prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
...
{
foreach($results as $result)
{   ?>                                               <input type="hidden"style="color: white;border: none;width: 0px;height:0px;" name="subjectname" class="form-control" id="default" placeholder="Enter your names or your position"  value="<?php echo htmlentities($result->classid);?>">
<input type="hidden"style="color: white;border: none;width: 0px;height:0px;" name="classid" class="form-control" id="default" placeholder="Enter your names or your position"  value="<?php echo htmlentities($result->UserName);?>">
                                                        <?php
}}?>
                                                    <label for="default" class="col-sm-2 control-label">Send to</label>
                                                    <div class="col-sm-10">
                                                        <select name="ongoing" class="form-control" id="default" >
